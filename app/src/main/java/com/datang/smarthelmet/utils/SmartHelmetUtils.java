...
/** Helpers. */
public final class SmartHelmetUtils {
    private static final Handler sHandler = new Handler(Looper.getMainLooper());

    private SmartHelmetUtils() {}
        ...
        if (!LinphonePreferences.instance().useJavaLogger()) {
            ...
            if (isDebugEnabled) {
                if (SmartHelmetService.isReady()) {
                    Factory.instance()
                            .getLoggingService()
                            .addListener(SmartHelmetService.instance().getJavaLoggingService());
                }
            } else {
                if (SmartHelmetService.isReady()) {
                    Factory.instance()
                            .getLoggingService()
                            .removeListener(SmartHelmetService.instance().getJavaLoggingService());
                }
            }
        }
    }
    ...
    public static boolean isNumberAddress(String numberOrAddress) {
        ProxyConfig proxy = SmartHelmetManager.getCore().createProxyConfig();
    ...
    public static String getDisplayableUsernameFromAddress(String sipAddress) {
        ...
        if (username.contains("@")) {
            ...
            if (lpc != null) {
                ...
                if (domain.equals(
                        SmartHelmetService.instance().getString(R.string.default_domain))) {
                    return username.split("@")[0];
                }
            }
        }
        ...
        call.setOnClickListener(
                new View.OnClickListener() {
                    ...
                    public void onClick(View view) {
                        CallLog[] logs = SmartHelmetManager.getCore().getCallLogs();
