...
public class ActivityMonitor implements Application.ActivityLifecycleCallbacks {
    ...
    void startInactivityChecker() {
        ...
        SmartHelmetUtils.dispatchOnUIThreadAfter((mLastChecker = new InactivityChecker()), 2000);
    }
    ...
    private void onBackgroundMode() {
        ...
        if (SmartHelmetManager.getCore() != null) {
            SmartHelmetManager.getCore().enterBackground();
        }
    }
    ...
    private void onForegroundMode() {
        ...
        if (SmartHelmetManager.getCore() != null) {
            SmartHelmetManager.getCore().enterForeground();
        }
    }
    ...
    class InactivityChecker implements Runnable {
        ...
        @Override
        public void run() {
            synchronized (SmartHelmetService.instance()) {
                if (!isCanceled) {
