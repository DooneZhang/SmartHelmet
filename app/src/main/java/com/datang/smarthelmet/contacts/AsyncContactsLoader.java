...
class AsyncContactsLoader extends AsyncTask<Void, Void, AsyncContactsLoader.AsyncContactsData> {
    ...
    protected void onPreExecute() {
        ...
        if (LinphonePreferences.instance().isFriendlistsubscriptionEnabled()) {
            ...
            for (FriendList list : SmartHelmetManager.getCore().getFriendsLists()) {
    ...
    protected AsyncContactsData doInBackground(Void... params) {
        ...
        HashMap<String, SmartHelmetContact> androidContactsCache = new HashMap<>();
        ...
        if (core != null) {
            ...
            for (FriendList list : friendLists) {
                ...
                for (Friend friend : friends) {
                    ...
                    SmartHelmetContact contact = (SmartHelmetContact) friend.getUserData();
                    if (contact != null) {
                        ...
                        if (friend.getRefKey() != null) {
                            // Friend has a refkey but no SmartHelmetContact => represents a
                            ...
                            contact = new SmartHelmetContact();
        ...
        if (c != null) {
            ...
            while (c.moveToNext()) {
                ...
                SmartHelmetContact contact = androidContactsCache.get(id);
                if (contact == null) {
                    Log.d(
                            "[Contacts Manager] Creating SmartHelmetContact with native ID "
                                    + id
                                    + ", favorite flag is "
                                    + starred);
                    nativeIds.add(id);
        ...
        Collection<SmartHelmetContact> contacts = androidContactsCache.values();
        Log.i("[Contacts Manager] Found " + contacts.size() + " contacts");
        for (SmartHelmetContact contact : contacts) {
            if (isCancelled()) {
                Log.w("[Contacts Manager] Task cancelled");
                return data;
            }
            ...
            if (contact.getFullName() == null) {
                for (SmartHelmetNumberOrAddress noa : contact.getNumbersOrAddresses()) {
                    if (noa.isSIPAddress()) {
                        contact.setFullName(SmartHelmetUtils.getAddressDisplayName(noa.getValue()));
                        Log.w(
    ...
    protected void onPostExecute(AsyncContactsData data) {
        ...
        for (SmartHelmetContact contact : data.contacts) {
        ...
        if (LinphonePreferences.instance().isFriendlistsubscriptionEnabled()) {
            ...
            FriendList[] friendLists = SmartHelmetManager.getCore().getFriendsLists();
            for (FriendList list : friendLists) {
    ...
    class AsyncContactsData {
        final List<SmartHelmetContact> contacts;
        final List<SmartHelmetContact> sipContacts;
