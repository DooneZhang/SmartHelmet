...
public class SmartHelmetContact extends AndroidContact
        implements Serializable, Comparable<SmartHelmetContact> {
    ...
    private List<SmartHelmetNumberOrAddress> mAddresses;
    private boolean mHasSipAddress;
    private boolean mIsStarred;

    public SmartHelmetContact() {
        super();
        mFullName = null;
        mFirstName = null;
        mLastName = null;
    ...
    public static SmartHelmetContact createContact() {
        SmartHelmetContact contact = new SmartHelmetContact();
    ...
    @Override
    public int compareTo(SmartHelmetContact contact) {
        ...
        if (fullName.equals(contactFullName)) {
            if (getAndroidId() != null) {
                if (contact.getAndroidId() != null) {
                    ...
                    List<SmartHelmetNumberOrAddress> noas1 = getNumbersOrAddresses();

                    List<SmartHelmetNumberOrAddress> noas2 = contact.getNumbersOrAddresses();
    ...
    public boolean equals(Object obj) {
        if (obj.getClass() != SmartHelmetContact.class) return false;
        SmartHelmetContact contact = (SmartHelmetContact) obj;
    ...
    */
    ...
    private synchronized void addNumberOrAddress(SmartHelmetNumberOrAddress noa) {
        if (noa == null) return;

        boolean found = false;
        // Check for duplicated phone numbers but with different formats
        for (SmartHelmetNumberOrAddress number : mAddresses) {
    ...
    public synchronized List<SmartHelmetNumberOrAddress> getNumbersOrAddresses() {
        return mAddresses;
    }

    public boolean hasAddress(String address) {
        for (SmartHelmetNumberOrAddress noa : getNumbersOrAddresses()) {
            if (noa.isSIPAddress()) {
    ...
    public synchronized void removeNumberOrAddress(SmartHelmetNumberOrAddress noa) {
        if (noa != null && noa.getOldValue() != null) {
            ...
            if (isFriend()) {
                ...
                SmartHelmetNumberOrAddress toRemove = null;
                for (SmartHelmetNumberOrAddress address : mAddresses) {
    ...
    public synchronized void addOrUpdateNumberOrAddress(SmartHelmetNumberOrAddress noa) {
    ...
    private void createFriend() {
        SmartHelmetContact contact = new SmartHelmetContact();
        Friend friend = SmartHelmetManager.getCore().createFriend();
        ...
        if (isFriend()) {
            ...
            if (core != null && core.vcardSupported()) {
                for (Address addr : mFriend.getAddresses()) {
                    if (addr != null) {
                        addNumberOrAddress(
                                new SmartHelmetNumberOrAddress(addr.asStringUriOnly(), true));
                    }
                }
                for (String tel : mFriend.getPhoneNumbers()) {
                    if (tel != null) {
                        addNumberOrAddress(new SmartHelmetNumberOrAddress(tel, false));
                    }
                }
            } else {
                Address addr = mFriend.getAddress();
                addNumberOrAddress(new SmartHelmetNumberOrAddress(addr.asStringUriOnly(), true));
            }
        }
    }
    ...
    public void save() {
        saveChangesCommited();
        syncValuesFromAndroidContact(SmartHelmetService.instance());
        createOrUpdateFriend();
    }
