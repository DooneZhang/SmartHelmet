...
import com.datang.smarthelmet.assistant.PhoneAccountLinkingAssistantActivity;
import com.datang.smarthelmet.call.CallManager;
...
import com.datang.smarthelmet.utils.MediaScanner;
...
public class SmartHelmetManager implements SensorEventListener {
    ...
    public SmartHelmetManager(Context c) {
        mExited = false;
        mContext = c;
    ...
    public static synchronized SmartHelmetManager getInstance() {
        SmartHelmetManager manager = SmartHelmetService.instance().getLinphoneManager();
        if (manager == null) {
    ...
    public static synchronized Core getCore() {
        if (getInstance().mExited) {
            ...
            // SmartHelmetManager was destroyed
    ...
    public synchronized void destroy() {
        ...
        // SmartHelmetManager.getCore(), for example to unregister a listener
        mExited = true;
    }
        ...
        try {
            ...
            TimerTask lTask =
                    new TimerTask() {
                        @Override
                        public void run() {
                            SmartHelmetUtils.dispatchOnUIThread(mIterateRunnable);
                        }
                    };
            /*use schedule instead of scheduleAtFixedRate to avoid iterate from being call in burst after cpu wake up*/
            mTimer = new Timer("SmartHelmet scheduler");
            mTimer.schedule(lTask, 0, 20);
        } catch (Exception e) {
            Log.e(e, "[Manager] Cannot start SmartHelmet");
        }
