package com.datang.smarthelmet.assistant;
...
public abstract class AssistantActivity extends SmartHelmetGenericActivity
    ...
    void createProxyConfigAndLeaveAssistant() {
        ...
        if (useLinphoneDefaultValues) {
            ...
            SmartHelmetService.instance().getNotificationManager().startForeground();
        }
    ...
    void goToLinphoneActivity() {
        boolean needsEchoCalibration =
                SmartHelmetManager.getCore().isEchoCancellerCalibrationRequired();
        ...
        if (needsEchoCalibration && !echoCalibrationDone) {
            ...
            boolean codecFound =
                    SmartHelmetManager.getInstance().getOpenH264DownloadHelper().isCodecFound();
