...
import com.datang.smarthelmet.purchase.Purchasable;
...
public class LinphonePreferences {
    ...
    private String getString(int key) {
        if (mContext == null && SmartHelmetService.isReady()) {
            mContext = SmartHelmetService.instance();
        }
    ...
    private Core getLc() {
        if (!SmartHelmetService.isReady()) return null;

        return SmartHelmetManager.getCore();
    }
    ...
    public void setAccountEnabled(int n, boolean enabled) {
        ...
        ProxyConfig prxCfg = getProxyConfig(n);
        if (prxCfg == null) {
            SmartHelmetUtils.displayErrorAlert(getString(R.string.error), mContext);
            return;
        }
        prxCfg.edit();
    ...
    public void setDebugEnabled(boolean enabled) {
        ...
        SmartHelmetUtils.configureLoggingService(enabled, mContext.getString(R.string.app_name));
    }

    public void setJavaLogger(boolean enabled) {
        getConfig().setBool("app", "java_logger", enabled);
        SmartHelmetUtils.configureLoggingService(
    ...
    public void setTunnelHost(String host) {
        ...
        if (config != null) {
            config.setHost(host);
            SmartHelmetManager.getInstance().initTunnelFromConf();
        }
    }
    ...
    public void enableDeviceRingtone(boolean enable) {
        ...
        SmartHelmetManager.getInstance().enableDeviceRingtone(enable);
    }
    ...
    public boolean enableTTS() {
        return getConfig().getBool("app", "shortcuts", true);
    }
}
